...
public class FileUpLoadChooserImpl implements IFileUploadChooser {
    ...
    private void openFileChooserInternal() {
        ...
        if (this.isAboveL && this.mFileChooserParams != null && this.mFileChooserParams.getAcceptTypes() != null) {
            ...
            for (String typeTmp : types) {
                ...
                if (AgentWebUtils.isImageMedia(typeTmp) || AgentWebUtils.isVideoMedia(typeTmp)) {
                    needCamera = true;
                    break;
                }
            }
        ...
        if (!TextUtils.isEmpty(this.acceptType)
                && !AgentWebUtils.isImageMedia(this.acceptType) && !AgentWebUtils.isVideoMedia(this.acceptType)) {
            touchOffFileChooserAction();
            return;
        }
        ...
        if (this.mAgentWebUIController.get() != null) {
            this.mAgentWebUIController
                    .get()
                    .showChooser(this.mWebView, mWebView.getUrl(), mFileUploadMsgConfig.getMedias(acceptType), getCallBack(acceptType));
            LogUtils.i(TAG, "open");
        }

    }

    private Handler.Callback getCallBack(String acceptType) {
        if (AgentWebUtils.isVideoMedia(acceptType)) {
    ...
    private void onRecordVideoAction() {
    ...
    private void openRecordVideoAction() {
        Action mAction = new Action();
        mAction.setAction(Action.ACTION_RECORD_VIDEO);
        ActionActivity.setFileDataListener(this.getFileDataListener());
        ActionActivity.start(mActivity, mAction);
    }
